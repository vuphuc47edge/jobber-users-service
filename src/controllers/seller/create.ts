...
import { BadRequestError, ISellerDocument } from '@vuphuc47edge/jobber-shared';
...
export const seller = async (req: Request, res: Response): Promise<void> => {
  ...
  res.status(StatusCodes.CREATED).json({ message: 'Seller created success.', seller: createdSeller });
};
